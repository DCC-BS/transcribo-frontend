    ...
    <div class="bg-linear-to-br">
        ...
        <div class="flex items-center justify-between p-4">
            ...
            <motion.div :whileHover="{ scale: 1.02 }" :whileTap="{ scale: 0.98 }"
                ...
                <UButton v-if="!props.transcription.summary" icon="i-lucide-lightbulb" variant="soft" color="warning"
                    size="sm" :label="isSummaryGenerating ? t('summary.generating') : t('summary.generate')"
                    :loading="isSummaryGenerating" :disabled="isSummaryGenerating" @click="handleGenerateSummary"
                    class="shadow-sm" />
                <UButton v-else icon="i-lucide-refresh-cw" variant="ghost" color="primary" size="sm"
        ...
        <!-- Error Alert -->
        <motion.div v-if="summaryError" :animate="{ opacity: 1, y: 0 }" :initial="{ opacity: 0, y: -10 }"
            :transition="{ type: 'spring', stiffness: 300, damping: 25 }" class="px-4 pb-4">
        ...
        <!-- Summary Content -->
        <motion.div v-if="props.transcription.summary" :animate="{ opacity: 1, height: 'auto' }"
            :initial="{ opacity: 0, height: 0 }" :transition="{ duration: 0.35, ease: 'easeOut' }">
        ...
        <!-- Empty State -->
        <motion.div v-else-if="!isSummaryGenerating" :animate="{ opacity: 1 }" :initial="{ opacity: 0 }"
