...
const speakers = computed(() =>
    Array.from(
        new Set(segments.value.map((segment) => segment.speaker ?? "unknown")),
    ),
);
...
const stageHeight = computed(
    () => speakers.value.length * heightPerSpeaker + marginTop,
);
...
function onDragMove(e: KonvaEventObject<MouseEvent, Rect>): void {
    ...
    const snappedLeft = checkSnap(leftEdge, [
        ...snapPoints.starts,
        ...snapPoints.ends,
    ]);

    // Check for snap on right edge (end time)
    const snappedRight = checkSnap(rightEdge, [
        ...snapPoints.starts,
        ...snapPoints.ends,
    ]);
...
function onTransform(e: KonvaEventObject<MouseEvent, Rect>): void {
    ...
    if (snappedLeft !== null) {
        const newWidth =
            rect.width() * rect.scaleX() + (leftEdge - snappedLeft);
        rect.x(snappedLeft);
        rect.width(newWidth);
